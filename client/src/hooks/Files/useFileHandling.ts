...
import store, { ephemeralAgentByConvoId, directAttachByConvoId } from '~/store';
...
const useFileHandling = (params?: UseFileHandling) => {
  ...
  const { files, setFiles, setFilesLoading, conversation, ask, index } = useChatContext();
  ...
  const startUpload = async (extendedFile: ExtendedFile) => {
    ...
    if (!isAssistantsEndpoint(endpointType ?? endpoint)) {
      // Normal file upload flow - works the same way whether Direct Attach is enabled or not
      // Direct Attach mode just means files are attached normally without RAG/indexing
      if (!agent_id) {
